...
@interface DAMyOrderViewController () <DADetailOrderDelegate> {
    ...
    NSMutableDictionary *printResult;
    int printerCount;
}
@end
...
//    self.dataList.items = [[NSArray alloc] init];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(printComplete:) name:@"print_callback" object:nil];
...
- (void)printComplete:(NSNotification *)notification {
    ...
    detailOrderVC.confirmCallback = ^(NSString *tips) {
        ...
        if (self.curService != nil) {
            [[DAOrderModule alloc] addOrder:orderList serviceId:self.curService._id deskId:deskId callback:^(NSError *err, DAMyOrderList *list) {
                ...
                if ([self.curService.type integerValue] == 3) {
                    printerCount = [DAPrintProxy addOrderPrintWithOrderList:self.dataList deskName:list.deskName orderNum:list.orderNum now:list.now takeout:self.curService.phone tips:@""];
                } else {
                ...
                if (printerCount == 0) {
                    ...
                    printResult = [[NSMutableDictionary alloc] init];
                }
